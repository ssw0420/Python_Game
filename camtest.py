...
class Game:
    ...
    def __init__(self):
        ...
        self.ingame_font_score = pg.font.Font(os.path.join(self.FontPath, "pdark.ttf"), int(self.width / 45))
    ...
    def draw_score(self):
        self.score_box = pg.draw.rect(self.screen, PLAY_COLOR, (self.play_x + self.play_width + 30, 250, 250, 100), self.hit_line)
        pg.display.update(self.score_box)
        self.score_text = self.ingame_font_score.render(str(self.score), False, WHITE)
        self.screen.blit(self.score_text, (self.play_x + self.play_width + 30, 250))
    ...
    def random_create_note(self):
        if self.Time > 5 and self.Time - 5 > 1.3 * self.create_note_time: # 노트 생성 주기
