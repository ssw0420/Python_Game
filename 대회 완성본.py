...
class Game:
    ...
    def __init__(self):
        ...
        # image2 = cv2.imread('Do_Doong.jpeg', 1)
        ...
        with self.mp_hands.Hands(
            ...
            while self.cap.isOpened():
                        ...
                        for res in results.multi_hand_landmarks:   
                            for j, lm in enumerate(res.landmark):
                                if j==8:
                                    ...
                                    if 0 <= lm.x*self.w and self.a >= lm.x*self.w: # 왼쪽 # 화면 반전을 안했기 때문.
                                        if self.hi * 2 <= lm.y*self.h:             # 3분할 선 아래
                                            self.__twoinit__()                     # 아래와 같이 새로 초기화를 시키고
                                            self.run()                             # 코드 종료

                                    elif self.a <lm.x*self.w and 2 * self.a >= lm.x*self.w: # 중앙
                                        if self.hi * 2 <= lm.y*self.h:          
                                            continue                               # 아무것도 실행 안됨.
                                
                                    else:
                                        if self.hi * 2 <= lm.y*self.h:             # 오른쪽
                                            sys.exit()                        # 게임 실행
                ...
                # roi = image[150: 150+rows, 150:150+cols]
                # mask = cv2.cvtColor(image2, cv2.COLOR_BGR2GRAY)
                # mask[mask[:]==255]=0
                # mask[mask[:]>0]=255
                # mask_inv = cv2.bitwise_not(mask)
                # image22 = cv2.bitwise_and(image2, image2, mask=mask)
                # back = cv2.bitwise_and(roi, roi, mask=mask_inv)
                # dst = cv2.add(image22, back)
                ...
                cv2.putText(image, "START", (100, 1000), cv2.FONT_HERSHEY_SIMPLEX, 5, (255, 255, 255))
                cv2.putText(image, "EXIT", (1400, 1000), cv2.FONT_HERSHEY_SIMPLEX, 5, (255, 255, 255))
                #보기 편하게 이미지를 좌우 반전합니다.

                # image[20:20+rows, 160:160+cols] = dst
    ...
    def start_screen(self):
        start_screen_open = self.cap.isOpened()

        while start_screen_open:
            success, image = self.cap.read()
            if not success:
                print("카메라를 찾을 수 없습니다.")
                # 동영상을 불러올 경우는 'continue' 대신 'break'를 사용합니다.
    ...
    def draw_score(self):
        self.score_box = pg.draw.rect(self.screen, BLACK, (self.play_x + self.play_width + 10, 230, 280, 330))
        self.score_box_edge = pg.draw.rect(self.screen, PLAY_COLOR, (self.play_x + self.play_width + 10, 230, 280, 330), 2)
        ...
        self.screen.blit(self.score_text_eng, (self.play_x + self.play_width + 150 - self.score_text_eng.get_width() / 2, 280 - self.score_text_eng.get_height() / 2))
    ...
    def draw_life(self):
        ...
        self.screen.blit(self.life_text_eng, (self.play_x + self.play_width + 150 - self.life_text_eng.get_width() / 2, 450 - self.life_text_eng.get_height() / 2))
        for i in range (self.life):
            self.life_box = pg.draw.rect(self.screen, (245 - (i * 18), 35 + (i * 5), 75 + (i * 8)), (self.play_x + self.play_width + 35 + (i * 23), 500, 23, 30))
    ...
    def random_create_note(self):
        if self.Time > 1.03 * self.create_note_time: # 노트 생성 주기
